...
const Vehicles = ({ pageSize = 12 }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`https://www.swapi.tech/api/vehicles?page=${page}&limit=${pageSize}`);
  ...
  return (
    <>
      <div className="row g-4">
        {items.map((v) => (
          <div className="col-12 col-sm-6 col-md-4 col-lg-3" key={`vehicles-${v.uid}`}>
            <ItemCard
              item={v}
              type="vehicles"
              onFav={() =>
                actions.toggleFavorite({ id: `vehicles-${v.uid}`, type: "vehicles", uid: v.uid, name: v.name })
              }
            />
          </div>
        ))}
      ...
      <div className="d-flex justify-content-center my-4">
        {!done ? (
          ...
          <span className="text-muted">No more vehicles</span>
        )}
      </div>
    </>
  );
